...
namespace Bga\Games\WizardsGrimoire;

$swdNamespaceAutoload = function ($class) {
    $classParts = explode('\\', $class);
    if ($classParts[0] == 'WizardsGrimoire') {
        array_shift($classParts);
        $file = dirname(__FILE__) . '/' . implode(DIRECTORY_SEPARATOR, $classParts) . '.php';
        if (file_exists($file)) {
            require_once $file;
        } else {
...
use WizardsGrimoire\Core\UtilsTrait;
...
class Game extends \Bga\GameFramework\Table {
    ...
    function __construct() {
        ...
        self::initGameStateLabels(array(
            ...
            WG_GAME_OPTION_SET => WG_GAME_OPTION_SET_ID,
        ));
    ...
    */
    protected function setupNewGame($players, $options = array()) {
        ...
        $cards_types = array_filter($this->card_types, function ($card_type) use ($gameOptionDifficulty, $gameOptionKickStarter1, $gameOptionShiftingSandPromo, $gameoptionSetBase, $gameoptionSetShiftingSand, $gameoptionSetAllCards) {
            ...
            } else if ($gameoptionSetShiftingSand) {
                switch ($card_type['icon']) {
                    case WG_ICON_SET_SHIFTING_SAND_1:
                        return true;
    ...
    */
    protected function getAllDatas(): array {
        ...
        $current_player_id = intval($this->getCurrentPlayerId());    // !! We must only return informations visible by this player !!
        ...
        switch(intval(self::getGameStateValue(WG_GAME_OPTION_SET))) {
            case WG_GAME_OPTION_SET_BASE:
                $result['card_set'] = 'base';
                break;
            case WG_GAME_OPTION_SET_SHIFTING_SAND:
    ...
    public static function getActiveDelayedSpellStates(int $phase, int $stActiveDelayedSpell, int $stSwithOpponent, int $stInteraction, int $stReturnPlayer, int $stActivePass) {
        return [
            $stActiveDelayedSpell => [
                ...
                "type" => "activeplayer",
                "possibleactions" => ["actActivateDelayedSpell", "actPass"],
                "transitions" => [
